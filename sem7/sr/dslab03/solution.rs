...
impl FibonacciModule {
    ...
    pub(crate) fn message(&mut self, idx: usize, num: Num) {
        ...
        // NOTE: w treści nie ma powiedziane, co zrobić, jeśli będzie overflow
        self.num = self.num.checked_add(num).expect("overflow");
        ...
        while let Ok(msg) = rx.recv() {
            match msg {
                ...
                FibonacciSystemMessage::Init { id, other } => {
                    ...
                    modules.get_mut(&other).unwrap().init(id);
                }
...
pub(crate) fn fib(n: usize) {
    ...
    let _ = tx.send(FibonacciSystemMessage::Init {
        id: fib1_id,
        other: fib2_id,
    });
